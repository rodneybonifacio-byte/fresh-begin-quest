      ...
      const fetchContador = async () => {
        try {
          const {
            data,
            error
          } = await supabase.from('contador_cadastros').select('contador, limite').eq('tipo', 'primeiros_cadastros').single();
          if (!error && data) {
        ...
        stiffness: 300
          ...
          <div className="relative h-40 bg-gradient-to-r from-primary via-orange-500 to-amber-500 overflow-hidden">
            ...
            <div className="absolute inset-0 overflow-hidden">
              {[...Array(20)].map((_, i) => <motion.div key={i} className="absolute w-3 h-3 rounded-full" style={{
          ...
          <div className="p-6 space-y-6">
            ...
            delay: 0.4
                ...
                {/* Shine Effect */}
                <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent skew-x-12" animate={{
              x: ['-200%', '200%']
            }} transition={{
              duration: 3,
              repeat: Infinity,
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-white/50">
                        {vagasRestantes > 0 ? `Restam apenas ${vagasRestantes} vagas!` : 'Promoção encerrada!'}
                      </span>
                      {vagasRestantes > 0 && vagasRestantes <= 20 && <motion.span animate={{
                    opacity: [1, 0.5, 1]
                  }} transition={{
                    duration: 1,
                    repeat: Infinity
            ...
            scale: 0.98
          }} onClick={onClose} className="w-full py-4 bg-gradient-to-r from-primary via-orange-500 to-amber-500 text-white font-bold rounded-xl shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 transition-all">
              Entendido! 👍
            </motion.button>
