  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950/20 to-slate-950 text-white">
      ...
      <header className="border-b border-white/5 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center font-bold">
              C+
            </div>
            <div>
              <h1 className="font-bold">Conecta+</h1>
              <p className="text-xs text-gray-400">Painel do Parceiro</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-400 hidden sm:block">
              Olá, <strong className="text-white">{parceiro.nome.split(' ')[0]}</strong>
            </span>
            <button
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8">
        {/* Link de Indicação */}
        <motion.div 
          className="mb-8 p-6 rounded-2xl bg-gradient-to-br from-purple-500/10 to-pink-500/5 border border-purple-500/20"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        >
          <h2 className="text-lg font-semibold mb-4">Seu Link de Indicação</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Link exclusivo</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  value={parceiro.link_indicacao}
                  readOnly
                  className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-gray-300"
                />
                <button
                ...
                >
                  {copied === 'link' ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5 text-purple-400" />}
                </button>
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Código de parceiro</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  value={parceiro.codigo_parceiro}
                  readOnly
                  className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-gray-300 font-mono"
                />
                <button
                  onClick={() => handleCopy('codigo', parceiro.codigo_parceiro)}
                  className="p-3 bg-purple-500/20 hover:bg-purple-500/30 rounded-xl transition-colors"
                >
                  {copied === 'codigo' ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5 text-purple-400" />}
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'clientes' && (
          ...
          >
            <div className="p-4 border-b border-white/5">
              <h3 className="font-semibold">Clientes Indicados</h3>
            </div>
            {clientes.length === 0 ? (
              <div className="p-12 text-center">
                <Users className="w-12 h-12 text-gray-600 mx-auto mb-4" />
                <p className="text-gray-400 mb-2">Nenhum cliente indicado ainda</p>
                <p className="text-sm text-gray-500">Compartilhe seu link para começar a ganhar!</p>
              </div>
            ) : (
                    ...
                    {clientes.map((cliente) => (
                      <tr key={cliente.id} className="hover:bg-white/5 transition-colors">
                        <td className="px-4 py-4">
                          <div>
                            <p className="font-medium">{cliente.cliente_nome || 'N/A'}</p>
                            <p className="text-xs text-gray-500">{cliente.cliente_email || '-'}</p>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 text-right text-sm font-medium text-green-400">
        ...
        {activeTab === 'overview' && (
          ...
          >
              ...
              {clientes.length === 0 ? (
                ...
                <div className="space-y-3">
                  {clientes.slice(0, 5).map((cliente) => (
                    <div key={cliente.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                      <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center">
                        <Users className="w-4 h-4 text-purple-400" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-medium">{cliente.cliente_nome || 'Novo cliente'}</p>
            ...
            <div className="p-6 rounded-2xl bg-white/5 border border-white/5">
              <h3 className="font-semibold mb-4">Dicas para Ganhar Mais</h3>
        ...
        {activeTab === 'config' && (
          ...
          >
            <div className="p-6 rounded-2xl bg-white/5 border border-white/5">
              <h3 className="font-semibold mb-6">Dados Bancários</h3>
        ...
        {activeTab === 'comissoes' && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
          >
            <h3 className="font-semibold mb-4">Extrato de Comissões</h3>
            ...
            <div className="mt-6 p-4 rounded-xl bg-purple-500/10 border border-purple-500/20">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">Dia 10 do próximo mês</p>
                </div>
              </div>
            </div>
          </motion.div>
