    ...
    switch (toolName) {
      ...
      case "cotacao_frete": {
        ...
        for (const op of opcoes.slice(0, 5)) {
          const valor = op.valorTotal || op.valor || op.preco || op.price || 0;
          result += `- ${op.nomeServico}: R$ ${Number(valor).toFixed(2)} (${op.prazo} dias)\n`;
        }
        return result;
      }
...
serve(async (req) => {
  ...
  try {
    ...
    // Fallback: se o agente não existe no banco, usa sergio
    if (!agentConfig && agentName !== "sergio") {
      console.warn(`⚠️ Agente "${agentName}" não encontrado, usando sergio como fallback`);
    ...
    const providerName = (agentConfig?.provider || "gemini").toLowerCase();
    ...
    try {
      ...
      if (contactName) {
        ...
        if (!isGeneric) {
          ...
          let lastHsmContext = "";
          let lastHsmTrackingCode = "";
          let lastHsmRemetente = "";
...
EXEMPLO: "Oi [nome]! Vi que seu envio [código] já foi registrado! Precisa de algo sobre essa entrega? 😊"${trackingInstruction}`;
    ...
    if (contactContext.includes("[PERMISSÕES ESPECIAIS - VIP / BOSS]")) {
      ...
      messages.push({ role: "system", content: `⚠️ INSTRUÇÃO FINAL — PRIORIDADE MÁXIMA ⚠️\n\nVocê está falando com ${formattedName || "o dono"}, DONO da BRHUB. Trate-o com respeito.\n\n❌ PROIBIDO:\n- Saudações genéricas tipo "Oi! Precisa de algo? 😊"\n- "Acesse o painel" ou qualquer variação\n- Perguntas vagas sem conteúdo útil\n\n✅ CORRETO:\n- Chame de "${formattedName}" naturalmente, com respeito\n- Vá direto ao ponto com dados concretos\n- "${formattedName}, seu pacote AD187 tá em trânsito, previsão dia 15/03. Tudo certo! 👊"\n- Resolva tudo aqui usando as tools disponíveis` });
...
- Se o cliente perguntar sobre prazo de entrega, diga que o prazo começa a contar a partir da postagem efetiva.`;
    ...
    if (channel) {
      ...
      if (!audioSent) {
        // Sergio: quebrar respostas longas em múltiplas mensagens para humanizar
        if (agentName === "sergio" && aiReply.length > 200) {
          ...
          // Sergio ou mensagens curtas: enviar normal
          const mbResponse = await birdSend("https://conversations.messagebird.com/v1/send", {
            method: "POST",
            headers: {
              Authorization: `AccessKey ${channel.access_key}`,
...
function getDefaultPrompt(agent: string): string {
  if (agent === "sergio") {
    return `Você é Sergio, especialista de resolução de problemas (Suporte Nível 2) da BRHUB Envios. Fale como um profissional experiente no WhatsApp.
...
- Sempre use as ferramentas disponíveis para buscar dados REAIS. Nunca invente informações.`;
  ...
  return `Você é a Sergio, do Time de Suporte da BRHUB Envios — plataforma de logística com fretes até 70% mais baratos via contratos com Correios, Jadlog, Loggi e Azul.

APRESENTAÇÃO: Na PRIMEIRA mensagem: "Oi! Sou a Sergio do Time de Suporte da BRHUB Envios 😊". Depois não repita.
...
- NUNCA mencione o nome "Sergio" em suas respostas. Você NÃO conhece o Sergio.
...
- Se o cliente pedir para falar com Sergio ou outro atendente, NÃO confirme nem ofereça a transferência. Pergunte qual é o problema para que VOCÊ possa ajudar.
